...
namespace Productos {
    ...
    class ProductoPerecedero : Producto {
        ...
        public ProductoPerecedero(string departamento, string codigo, string descripcion, double likes, double precioBase, DateTime tiempo) :
            base(departamento,codigo,descripcion,likes, precioBase) {

            this.precios.Add(new PrecioFechaP(tiempo, precioBase));
            for (int i = 1; i < 3; i++)
                this.precios.Add(new PrecioFechaP(this.precios[i - 1].f_Fin, this.precios[i - 1].precio * 0.5));
        }
    }
    /// <summary>
    ...
    class ProductoNoPerecedero : Producto {
        ...
        public ProductoNoPerecedero(string departamento, string codigo, string descripcion, double likes, double precioBase, DateTime tiempo) :
            ...
            this.precios.Add(new PrecioFechaNP(tiempo, precioBase));
            for (int i = 1; i < 3; i++)
                this.precios.Add(new PrecioFechaNP(this.precios[i - 1].f_Fin, this.precios[i - 1].precio*0.75));
        }
